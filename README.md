...
Once the dictionary is loaded, the algorithm works by iterating over the input, line by line. Each line is subsequently tokenised by whitespace, disregarding contiguous whitespace sequences. E.g., the line `"To be...    or   not to be!"` is captured as six words: `["To", "be...", "or", "not", "to", "be!"]`.

Each resulting word is subject to three rule-sets — punctuation, compaction and capitalisation — applied in that order.

The **punctuation rule** is used to efficiently deal with punctuated words such as `"banana!"`. Normally, `"banana"` would be an easy dictionary lookup, but the exclamation mark is a fly in the ointment. Punctuation splits every word into a _prefix_ and a _suffix_. The prefix encompasses a contiguous string of alphabetical characters from the start of the word. The suffix contains all other characters. E.g., for `"bananas!!!"`, the punctuation tuple is `("bananas", "!!!")`. For an unpunctuated word, the prefix encompasses the entire word, while the suffix is an empty string; e.g., `"pear"` splits to `("pear", "")`. 
...
The **capitalisation** rules encode capitalisation _hints_ into the output so that the word-prefix may have its capitalisation restored. This rule is not perfectly reversible, but copes well in the overwhelming majority of cases.

1. If the original word-prefix starts with a capital letter, then the resulting output also has its first letter capitalised.
2. If the original word-prefix contains capitals in any letter but the first, then the entire output is capitalised.

The encoded output of each prefix is subsequently combined with the suffix to form the encoded word.
...
For each tokenised word, we apply the punctuation rule first, followed by reverse-compaction, then by capitalisation.
